...
describe("processSalahs", () => {
    ...
    it(`Next salah ${Constants.SALAH_NAMES[1]}. azanCalledDateTime undefined. now after ${Constants.SALAH_NAMES[5]}, before ${Constants.SALAH_NAMES[1]} azan.`, () => {
        ...
        // Call
        let result = processSalahs(DateCreator.fromISO(TODAY_DATE_STR + "T08:01"), fakeSalahs, undefined);
        // Assert
        expect(result.mainMessage).toBe(`Next salah: ${Constants.SALAH_NAMES[1]}`);

        // Call
        result = processSalahs(DateCreator.fromISO(TODAY_DATE_STR + "T10:00"), fakeSalahs, undefined);
        // Assert
        expect(result.mainMessage).toBe(`Next salah: ${Constants.SALAH_NAMES[1]}`);

        // Call
        result = processSalahs(DateCreator.fromISO(TODAY_DATE_STR + "T12:59"), fakeSalahs, undefined);
        // Assert
        expect(result.mainMessage).toBe(`Next salah: ${Constants.SALAH_NAMES[1]}`);
    });
});